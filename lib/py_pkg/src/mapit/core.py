...
scanning_poses = [
    ...
    },
    {
        ...
        "tool_pose": [
            -0.023442205755523776,
            -0.33657555977862413,
            0.46867671390885335,
            -0.010642740577652462,
            -2.9954775886052403,
            0.8561700796562706,
        ],
    },
...
camera = CalibratedCamera.new_from_config(
    "calibrated_camera.yml", get_realsense_frame(pipeline)
)
...
with open(
    "/home/jesper/DTU/KAND/robotic_lab_sbs_mapper/lib/py_pkg/src/mapit/close.script"
) as f:
    close_gripper = f.read()

with open(
    "/home/jesper/DTU/KAND/robotic_lab_sbs_mapper/lib/py_pkg/src/mapit/open.script"
) as f:
    open_gripper = f.read()
...
def refine(detected_ids):
    ...
    for id in detected_ids:
        ...
        desired_rotvec = d.align_camera_to_point(
            origin=origin,
            target=marker.position.reshape(
                3,
            ),
            R_tcp2base=R_tcp2base,
            T_tcp2base=T_tcp2base.reshape(
                3,
            ),
        )
        ...
        distance = np.sqrt(
            np.power(marker.position[0] - T_tcp2base[0], 2)
            + np.power(marker.position[1] - T_tcp2base[1], 2)
            + np.power(marker.position[2] - T_tcp2base[2], 2)
        )
        z = distance - 0.25
        v = np.array([[0], [0], [1]])
        ...
        refined_orientation = rvecs[:, :3].mean(
            axis=0
        )  # TODO does not make mathematical sense to take the mean of a rotation vector
...
# compute correlation
H_plate_in_mantis = apply_transformation(
    H_plate_in_base, np.linalg.inv(H_mantis_in_base)
)
...
fix_z = np.arccos(
    np.dot(rvec1, rvec2) / (np.linalg.norm(rvec1) * np.linalg.norm(rvec2))
)
