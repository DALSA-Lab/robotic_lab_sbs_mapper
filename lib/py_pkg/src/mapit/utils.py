...
def make_homogeneous(R_matrix: np.ndarray, t_vector: np.ndarray) -> np.ndarray:
    ...
    ----------
    R_matrix : numpy.array
        A 3x3 rotation matrix or 3x1 rotation vector.
    t_vector : numpy.array
    ...
    assert R_matrix.shape == (3, 3), (
        "R_matrix has incorrect shape, should be 3x3 or 3x1"
    )

    H = np.block([[R_matrix, t_vector.reshape(-1, 1)], [0, 0, 0, 1]])
    return H
...
def apply_transformation(target: np.ndarray, transformation: np.ndarray) -> np.ndarray:
    ...
    assert transformation.shape == (4, 4), (
        "transformation has incorrect shape, should be 4x4"
    )
    assert target.shape == (4, 4) or target.shape == (3,), (
        f"target has incorrect shape{target.shape}, should be either 4x4 or 3x1"
    )

    # if the target shape is a 3D point, expand the dimension by one before applying the transformation
    if target.shape == (3,):
...
def extract_pose_and_orientation(H_matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    ------
    AssertionError
        If the shape of the input matrix is not 4x4.
...
def build_pose(tvec, rvec):
    ...
    ----------
    tvec : array
        A 1x3 array representing a 3D position.
    rvec : array
        A 1x3 array representing the rotation vector.
...
def rot_tran_from_tool_pose(tool_pose: ur_commander.TaskPose):
    ...
    Parameters
    ----------
    tool_pose : ur_commander.TaskPose
        A 1x6 array containg a translation and rotation vector.
    ...
    ------
    AssertionError
        If the size of the input tool_pose is not 6.

    """
    assert len(tool_pose) == 6, (
        "Invalid tool_pose. Unable to extract rotation and translation."
    )
...
def axis_angle_align(camera_position, target, R_cam2base):
    ...
    if np.linalg.norm(N) < 1e-6:
        if (
            np.dot(v0, v1) > 0
        ):  # Vectors are parallel and pointing same way (0 rotation)
            return np.eye(3)
