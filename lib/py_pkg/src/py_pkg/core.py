...
R_cam2tcp = np.array([
    [0.7056748363153558, -0.7083972134537394, -0.01401475516279415],
 [0.7085287887478512, 0.7054415682355211, 0.0184160071924236],
 [-0.003159257317578533, -0.02292557036118817, 0.9997321827953802]

], dtype=np.float64)

T_cam2tcp = np.array( [0.01219444443892027,
 -0.06147861320325889,
 0.01941077308057536], dtype=np.float64
)
...
camera_intrinsics = np.array([
    [1358.6121477996473, 0., 981.08897945094088],
    [       0.,1358.7800907842454, 574.26822147400958],
    [        0., 0., 1. ]
], dtype=np.float64)

distortion_coefficients = np.array([
     0.048974893698110318, 0.34420835847301512,
       -0.00032343468545663714, 0.0019533087013580445,
       -1.4648353743713258
], dtype=np.float64)
...
for poses in scanning_poses:
    ...
    # for each id, compute the initial pose
    for index, id in enumerate(ids):
        print("target ID:", id) 
        
        img_points = corners[index]
...
for id in global_ids:
    ...
    pose = build_pose(v_in_base, desired_rotvec)    
    robot.movej(TaskPose(pose), blocking=True)
    rawframes = pipeline.wait_for_frames()
    raw_color_frame = rawframes.get_color_frame()
